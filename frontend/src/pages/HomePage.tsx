...
const MODELS = [
  { icon: Globe2,       name: 'Mithuna-Global',   full: 'NCMRWF Unified Model – Global',         badge: '12 km · 10 days',      desc: 'Global deterministic NWP at 12 km resolution, running four times daily (00/06/12/18 UTC), adapted from UK Met Office.' },
  { icon: Wind,         name: 'Mithuna-Regional',   full: 'NCMRWF Unified Model – Regional',       badge: 'Regional · High-res',   desc: 'High-resolution regional model over the Indian subcontinent for finer-scale short-range weather guidance.' },
  { icon: BarChart3,    name: 'Mithuna-Ensemble',   full: 'NCMRWF Ensemble Prediction System',     badge: 'Probabilistic',         desc: 'Ensemble system using multiple perturbed initial conditions for probabilistic forecasts and uncertainty quantification.' },
  { icon: Cloud,        name: 'Mithuna-Coupled',   full: 'Coupled Global NWP System',             badge: 'Ocean–Atmosphere',      desc: 'Fully coupled atmosphere–ocean–sea-ice model enabling improved monsoon and cyclone forecast skill.' },
...
export default function HomePage() {
  ...
  return (
    <div className="bg-[#f0f5fc]">
          ...
          <div className="grid lg:grid-cols-2 gap-16 items-center">
            <div className="animate-fade-up">
              ...
              <p className="text-sky-200/80 font-sans text-sm leading-relaxed mb-8 max-w-lg">
                Hosted by NCMRWF & IMS Noida Chapter, Ministry of Earth Sciences. Uniting physics-driven science and Artificial Intelligence for the next generation of tropical forecasting.
              </p>
          ...
          <div className="grid lg:grid-cols-2 gap-16 items-start">
            <div>
              ...
              <p className="text-slate-600 font-sans text-base leading-relaxed mb-4">
                ...
                This year, TROPMET will be jointly hosted by <strong className="text-[#0b2d5e]">NCMRWF, MoES</strong> and the <strong className="text-[#0b2d5e]">IMS Noida Chapter</strong>, Ministry of Earth Sciences, Noida during <strong className="text-[#0b2d5e]">19–21 November 2026</strong>.
              </p>
      ...
      <section className="py-24 bg-gradient-to-br from-[#021b3e] via-[#0b2d5e] to-[#0e3d7a] relative overflow-hidden">
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6">
          <div className="grid lg:grid-cols-2 gap-16 items-center">
            ...
            {/* ── Video Player ── */}
            <div className="relative rounded-2xl overflow-hidden border border-sky-400/25 shadow-2xl bg-black group">
              <video
                id="ncmrwf-about-video"
                ...
                onPlay={() => {
                  const btn = document.getElementById('ncmrwf-play-btn');
              ...
              >
                <source src="/videos/Tropmet_Slides_Website.mp4" type="video/mp4" />
              ...
              <button
                ...
                onClick={() => {
                  const vid = document.getElementById('ncmrwf-about-video') as HTMLVideoElement | null;
                  if (!vid) return;
                  if (vid.paused) {
                    vid.play();
                    const btn = document.getElementById('ncmrwf-play-btn');
              ...
              >
                <div className="w-16 h-16 rounded-full bg-amber-400/90 flex items-center justify-center shadow-xl hover:bg-amber-300 hover:scale-110 transition-all duration-200">
                  {/* play triangle */}
                  <svg viewBox="0 0 24 24" fill="currentColor" className="w-7 h-7 text-[#021b3e] ml-1">
              ...
              <div className="absolute bottom-0 left-0 right-0 h-10 bg-gradient-to-t from-black/60 to-transparent rounded-b-2xl pointer-events-none" />
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="py-24 bg-[#f0f5fc] border-b border-[#c8d9f0]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          <div className="max-w-3xl mb-14">
            <p className="font-sans text-xs font-bold uppercase tracking-[.18em] text-amber-500 mb-2">MITHUNA Operations</p>
      ...
      <section className="py-16 bg-[#f0f5fc] border-b border-[#c8d9f0]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          <div className="text-center mb-10">
            <p className="font-sans text-xs font-bold uppercase tracking-[.18em] text-amber-500 mb-2">Supporting Institutions</p>
            <h2 className="font-display text-3xl font-bold text-[#0b2d5e] mb-3">Institutional Patrons</h2>
            <div className="w-10 h-0.5 bg-amber-500 mx-auto" />
          </div>
          <div className="grid sm:grid-cols-2 gap-6 max-w-3xl mx-auto mb-8">
            {[
              { short: 'NCMRWF', name: 'National Centre for Medium Range Weather Forecasting', logo: '/src/assets/ncmrwf-logo-new.png' },
              { short: 'IMD',    name: 'India Meteorological Department',                      logo: '/src/assets/emblem-india.svg'    },
            ].map((p) => (
              <Link key={p.short} to="/institutional-patrons"
                className="group bg-white border border-[#c8d9f0] rounded-2xl p-6 shadow-sm hover:shadow-lg hover:border-[#0b2d5e]/40 transition-all flex flex-col items-center text-center">
                <div className="w-20 h-20 bg-[#f0f5fc] rounded-xl flex items-center justify-center p-3 mb-4 border border-[#c8d9f0] group-hover:border-amber-300 transition-colors">
                  <img src={p.logo} alt={p.short} className="w-full h-full object-contain" />
                </div>
                <span className="text-[11px] font-bold uppercase tracking-widest bg-amber-100 text-amber-700 px-2.5 py-1 rounded-full mb-2">{p.short}</span>
                <p className="font-sans font-semibold text-[#0b2d5e] text-sm leading-snug">{p.name}</p>
                <span className="mt-3 text-xs text-[#1a5fa8] font-semibold flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
          ...
          <div className="text-center">
            <Link to="/institutional-patrons"
              className="inline-flex items-center gap-2 border border-[#0b2d5e] text-[#0b2d5e] hover:bg-[#0b2d5e] hover:text-white text-sm px-6 py-2.5 rounded-lg font-bold transition-colors">
              View All Patrons <ChevronRight size={14} />
            </Link>
          </div>
        </div>
      </section>
      ...
      <section className="py-24 bg-white border-b border-[#c8d9f0]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          ...
          <div className="text-center mb-10">
            ...
            <div className="w-12 h-1 bg-gradient-to-r from-amber-400 to-amber-200 rounded-full mx-auto" />
          </div>

          {/* 2. Full-width HPC photo */}
          <div className="relative rounded-2xl overflow-hidden shadow-2xl mb-0">
          ...
          {/* 3. Stats bar — directly below the image */}
          <div className="bg-[#021b3e] rounded-b-2xl shadow-xl mb-10 overflow-x-auto">
            <div className="flex items-center justify-between gap-0 min-w-max md:min-w-0 divide-x divide-white/10">
              {[
                { value: '~8.24', label: 'PetaFLOPS Peak' },
                { value: '2,115', label: 'CPU Nodes' },
                { value: '18',    label: 'GPU Nodes (A100)' },
                { value: '~0.8',  label: 'PetaFLOPS GPU' },
                { value: '20 PB', label: 'NL-SAS Storage' },
                { value: '2 PB',  label: 'NVMe Scratch' },
                { value: '2.295 MW', label: 'Power Capacity' },
                { value: '~1.2',  label: 'PUE' },
              ].map((stat, i) => (
                <div key={i} className="flex-1 text-center px-4 py-4">
                  <div className="text-amber-400 font-display text-lg md:text-xl font-bold whitespace-nowrap">{stat.value}</div>
                  <div className="text-sky-300 text-xs font-sans font-semibold uppercase tracking-wide whitespace-nowrap mt-0.5">{stat.label}</div>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="text-center mb-8">
            <p className="text-slate-600 font-sans text-base max-w-3xl mx-auto leading-relaxed">
              Arunika is NCMRWF's flagship HPC system, inaugurated by the Hon'ble Prime Minister of India.
              Ranked 3<sup>rd</sup> on <strong>TopSC.in</strong> (India's Top Supercomputers list), it underpins
              national numerical weather prediction and supports MoES partner organisations.
            </p>
          </div>
