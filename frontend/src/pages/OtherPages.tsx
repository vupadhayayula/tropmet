      ...
      <div className="bg-gradient-to-r from-[#021b3e] to-[#0b2d5e] py-16 border-b border-sky-700/30">
        <div className="max-w-4xl mx-auto px-4 sm:px-6">
          ...
          <p className="text-sky-300 font-sans text-sm leading-relaxed">
            National Conference on Physics-driven and AI-enabled Research for Advanced Guidance and Tropical Intelligence
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 mt-12 space-y-8">
        <div className="bg-white border border-[#c8d9f0] rounded-2xl p-8 shadow-sm">
          ...
          <p className="font-sans text-base text-slate-700 leading-relaxed mb-2">
            ...
            Ministry of Earth Sciences, Sector-62, Noida, UP
          </p>
          <p className="font-sans text-base text-slate-700 leading-relaxed">
            ...
            — under the auspices of the Indian Meteorological Society (IMS).
          </p>
        </div>
...
export function CommitteesPage() {
  ...
  const localOrganizing = [
    { name: 'Dr. Raghavendra Ashrit',       designation: 'SC-G, NCMRWF',                                    role: 'Chairman'    },
    { name: 'Dr. Ashish Routray',           designation: 'Chairman, IMS Noida Chapter & SC-G, NCMRWF',      role: 'Co-Chairman' },
    { name: 'Dr. Indira Rani',              designation: 'Sc-F, NCMRWF',                                    role: ''            },
    { name: 'Dr. Anumeha Dube',             designation: 'Sc-E, NCMRWF',                                    role: ''            },
    { name: 'Dr. K. Niranjan Kumar',        designation: 'Sc-E, NCMRWF',                                    role: ''            },
    { name: 'Dr. Suryakanti Dutta',         designation: 'Sc-E, NCMRWF',                                    role: ''            },
    { name: 'Dr. Akhilesh Kumar Mishra',    designation: 'IMS Noida / MoES, SC-E, NCMRWF',                  role: ''            },
    { name: 'Prof. Sushil Kumar',           designation: 'IMS Noida / GBU, Greater Noida',                  role: ''            },
    { name: 'Dr. D. Srinivas',              designation: 'Secretary, IMS Noida Chapter & SC-D, NCMRWF',     role: 'Convener'    },
  ]
    ...
    <div className="min-h-screen bg-[#f0f5fc] pt-28 pb-16">
      ...
      <div className="bg-gradient-to-r from-[#021b3e] to-[#0b2d5e] py-16 border-b border-sky-700/30">
        <div className="max-w-5xl mx-auto px-4 sm:px-6">
          ...
          <div className="w-10 h-0.5 bg-amber-400 mb-4" />
          <p className="text-sky-300 font-sans text-sm">Local Organizing, National Organizing & Scientific Committees of TROPMET 2026</p>
        </div>
      </div>

      <div className="max-w-6xl mx-auto px-4 sm:px-6 mt-12 space-y-14">
        ...
        <div>
          <div className="flex items-center gap-3 mb-8">
            ...
            <div>
              <h2 className="font-display text-2xl font-bold text-[#0b2d5e]">Local Organizing Committee</h2>
              <p className="text-slate-500 text-sm font-sans">NCMRWF & IMS Noida Chapter</p>
            </div>
          </div>

          {/* Chairman & Co-Chairman — featured top row */}
          <div className="grid sm:grid-cols-2 gap-5 mb-5">
            {localOrganizing.filter(m => m.role === 'Chairman' || m.role === 'Co-Chairman').map((m, i) => (
              <div key={i} className="bg-white border-2 border-amber-300 rounded-2xl p-6 shadow-md hover:shadow-lg transition-all flex flex-col items-center text-center">
                <div className="w-16 h-16 bg-gradient-to-br from-[#021b3e] to-[#1a5fa8] rounded-full flex items-center justify-center text-white font-bold text-xl shadow-lg mb-4">
                  {m.name.replace('Dr. ', '').replace('Prof. ', '').trim().split(' ').map((w: string) => w[0]).slice(0, 2).join('')}
                </div>
                <span className="text-[11px] font-bold uppercase tracking-widest bg-amber-100 text-amber-700 border border-amber-300 px-3 py-1 rounded-full mb-3">
                  {m.role}
                </span>
                <h3 className="font-display font-bold text-[#0b2d5e] text-base mb-1">{m.name}</h3>
                <p className="text-slate-500 text-xs leading-relaxed">{m.designation}</p>
              </div>
            ))}
          </div>

          {/* Members — 3-column card grid */}
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-5">
            {localOrganizing.filter(m => !m.role).map((m, i) => (
              <div key={i} className="bg-white border border-[#c8d9f0] rounded-2xl p-5 shadow-sm hover:shadow-md hover:border-[#0b2d5e]/30 transition-all flex flex-col items-center text-center">
                <div className="w-12 h-12 bg-gradient-to-br from-[#0b2d5e] to-[#1a5fa8] rounded-full flex items-center justify-center text-white font-bold text-sm shadow mb-3">
                  {m.name.replace('Dr. ', '').replace('Prof. ', '').trim().split(' ').map((w: string) => w[0]).slice(0, 2).join('')}
                </div>
                <h3 className="font-display font-bold text-[#0b2d5e] text-sm mb-1">{m.name}</h3>
                <p className="text-slate-500 text-xs leading-relaxed">{m.designation}</p>
              </div>
            ))}
          </div>

          {/* Convener — dark accent full-width card */}
          {localOrganizing.filter(m => m.role === 'Convener').map((m, i) => (
            <div key={i} className="bg-gradient-to-r from-[#021b3e] to-[#0b2d5e] rounded-2xl p-6 shadow-lg flex flex-col sm:flex-row items-center gap-5 text-center sm:text-left">
              <div className="w-16 h-16 shrink-0 bg-white/10 border-2 border-amber-400/50 rounded-full flex items-center justify-center text-white font-bold text-xl shadow-lg">
                {m.name.replace('Dr. ', '').replace('Prof. ', '').trim().split(' ').map((w: string) => w[0]).slice(0, 2).join('')}
              </div>
              <div className="flex-1">
                <span className="text-[11px] font-bold uppercase tracking-widest bg-amber-400/20 text-amber-300 border border-amber-400/30 px-3 py-1 rounded-full inline-block mb-2">Convener</span>
                <h3 className="font-display font-bold text-white text-base mb-1">{m.name}</h3>
                <p className="text-sky-300 text-xs leading-relaxed">{m.designation}</p>
              </div>
            </div>
          ))}
        </div>
        ...
        <div>
          <div className="flex items-center gap-3 mb-6">
            <div className="w-1 h-10 rounded-full bg-[#1a5fa8]" />
            <div>
              <h2 className="font-display text-2xl font-bold text-[#0b2d5e]">National Organizing Committee</h2>
              <p className="text-slate-500 text-sm font-sans">Representatives from national institutions</p>
            </div>
          </div>
          <div className="bg-white border border-[#c8d9f0] rounded-2xl p-10 flex flex-col items-center justify-center text-center shadow-sm min-h-[160px]">
            <div className="w-12 h-12 bg-[#e8f0fb] rounded-full flex items-center justify-center mb-4">
              <svg className="w-6 h-6 text-[#1a5fa8]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
            </div>
            <p className="font-display font-bold text-[#0b2d5e] text-lg mb-1">To Be Announced Soon</p>
            <p className="text-slate-500 text-sm font-sans">The National Organizing Committee members will be announced shortly.</p>
          </div>
        </div>
        ...
        <div>
          <div className="flex items-center gap-3 mb-6">
            ...
            <div>
              <h2 className="font-display text-2xl font-bold text-[#0b2d5e]">Scientific Committee</h2>
              <p className="text-slate-500 text-sm font-sans">Domain experts guiding the scientific programme</p>
            </div>
          </div>
          <div className="bg-white border border-[#c8d9f0] rounded-2xl p-10 flex flex-col items-center justify-center text-center shadow-sm min-h-[160px]">
            ...
            <p className="font-display font-bold text-[#0b2d5e] text-lg mb-1">To Be Announced Soon</p>
            <p className="text-slate-500 text-sm font-sans">The Scientific Committee members will be announced shortly.</p>
          </div>
        </div>
        ...
        <div className="bg-amber-50 border border-amber-200 rounded-2xl p-6">
          <p className="text-amber-800 font-sans text-sm leading-relaxed">
            The full committee listing will be updated as confirmations are received. For inquiries, contact{' '}
...
export function CallForPapersPage() {
  const workflowSteps = [
    ...
    { step: 2, title: 'Abstract Submission',            desc: 'Register as an author, then login to your dashboard and submit your abstract (max 350 words) by 15 August 2026.',  action: 'Register & Submit', link: '/author-registration',  color: 'from-teal-700 to-teal-500' },
    ...
    { step: 5, title: 'Confirmation of Participation',  desc: 'Accepted authors confirm participation via their author dashboard by 15 September 2026.',                       action: 'My Dashboard',    link: '/author-dashboard',     color: 'from-orange-600 to-orange-500' },
    { step: 6, title: 'Online Registration & Payment',  desc: 'Pay the conference fee via your author dashboard. Early Bird rates apply through 31 October 2026.',            action: 'My Dashboard',    link: '/author-dashboard',     color: 'from-green-700 to-green-500' },
    { step: 7, title: 'Confirmation & ID Card',         desc: 'After successful payment you receive a confirmation email and a downloadable ID card — available in your author dashboard.',  action: 'My Dashboard',  link: '/author-dashboard',     color: 'from-rose-700 to-rose-500' },
  ]
  return (
    <div className="min-h-screen bg-[#f0f5fc] pt-28 pb-16">
      <div className="bg-gradient-to-r from-[#021b3e] to-[#0b2d5e] py-16 border-b border-sky-700/30">
...
const NOIDA_PLACES = [
  ...
  },
  {
    ...
    importance: 'An iconic war memorial standing at the heart of New Delhi, surrounded by lush lawns and fountains. A symbol of national pride and a landmark every visitor to Delhi must see.',
  ...
  },
  {
    ...
    importance: 'A magnificent 17th-century Mughal fort and UNESCO World Heritage Site in Old Delhi. Its imposing red sandstone walls, palaces, and museums narrate centuries of Indian history.',
  ...
  },
  {
    name: 'Qutub Minar',
    category: 'UNESCO Heritage',
    importance: 'The world\'s tallest brick minaret and a stunning UNESCO World Heritage Site in South Delhi. Built in the 12th century, it is a marvel of Indo-Islamic architecture and craftsmanship.',
  ...
  },
  {
    ...
    importance: 'Mathura, the birthplace of Lord Krishna, is one of India\'s holiest cities with vibrant ghats and ancient temples. Nearby Vrindavan, on the banks of the Yamuna, is dotted with over 5,000 temples including the iconic Banke Bihari Temple and ISKCON Vrindavan — together a timeless pilgrimage and cultural experience.',
  ...
  },
  {
    ...
    importance: 'One of the Seven Wonders of the World and a UNESCO Heritage Site, the Taj Mahal is a breathtaking monument of eternal love built by Emperor Shah Jahan. A sunrise visit is unforgettable.',
    image: 'https://images.unsplash.com/photo-1564507592333-c60657eea523?w=600&q=80',
    emoji: '🕌',
    tag: 'Nearby',
  },
  {
    name: 'Agra Fort',
    category: 'UNESCO Heritage',
    importance: 'A massive 16th-century Mughal fortress in Agra, just 2.5 km from the Taj Mahal. Its grand palaces, audience halls, and commanding views of the Yamuna make it a world-class historical site.',
    image: 'https://images.unsplash.com/photo-1548013146-72479768bada?w=600&q=80',
    emoji: '🏰',
    tag: 'Nearby',
  },
  {
    name: 'PM Sangrahalaya (Prime Ministers\' Museum)',
    category: 'National Museum',
    importance: 'India\'s state-of-the-art museum at Teen Murti Estate, New Delhi, chronicling the lives and contributions of all Prime Ministers since independence. With interactive exhibits, archival footage, and immersive displays, it is a must-visit to understand modern India\'s democratic journey.',
    image: 'https://images.unsplash.com/photo-1587474260584-136574528ed5?w=600&q=80',
    emoji: '🏛️',
    tag: 'Delhi',
  },
    ...
    <div className="min-h-screen bg-[#f0f5fc]">
      ...
      <div className="bg-gradient-to-br from-[#0b2d5e] to-[#1a5fa8] pt-32 pb-16 px-4 sm:px-6">
        <div className="max-w-4xl mx-auto text-center">
          ...
          <h1 className="font-display text-4xl md:text-5xl font-bold text-white mb-4">Places to Explore · Delhi & Noida</h1>
          ...
          <p className="text-sky-200 font-sans text-base max-w-2xl mx-auto leading-relaxed">
            Make the most of your visit to TROPMET 2026. Noida, Delhi, and nearby heritage destinations like Vrindavan, Mathura, and Agra offer a rich tapestry of nature, culture, and history.
          </p>
        </div>
      </div>
...
export function InstitutionalPatronsPage() {
  const patrons = [
    {
      short: 'NCMRWF',
      name: 'National Centre for Medium Range Weather Forecasting',
      ministry: 'Ministry of Earth Sciences, Government of India',
      logo: '/src/assets/ncmrwf-logo-new.png',
      accent: '#0b2d5e',
      website: 'https://www.ncmrwf.gov.in',
      description: [
        'The National Centre for Medium Range Weather Forecasting (NCMRWF) is a premier research and operational institute under the Ministry of Earth Sciences (MoES), Government of India. Established in 1988, it is located in Noida, Uttar Pradesh.',
        'NCMRWF operates state-of-the-art Numerical Weather Prediction (NWP) models and provides medium-range weather forecasts for the Indian subcontinent. It runs global and regional models, including the NCUM (NCMRWF Unified Model) family, S2S prediction systems, and the Hurricane Analysis and Forecast System (HAFS) for tropical cyclone guidance.',
        "The centre houses Arunika, one of India's most powerful supercomputers with a peak performance of ~8.24 PetaFLOPS, supporting advanced weather simulations, AI/ML research, and climate modelling. NCMRWF collaborates with national and international agencies including IMD, IITM, ECMWF, and the UK Met Office.",
        'NCMRWF plays a critical role in disaster preparedness, providing real-time forecasts to the National Disaster Management Authority (NDMA), State governments, agricultural agencies, and ISRO for space launch support.',
      ],
      highlights: [
        { label: 'Established', value: '1988' },
        { label: 'Location', value: 'Noida, UP' },
        { label: 'Under', value: 'MoES, GoI' },
        { label: 'Supercomputer', value: 'Arunika ~8.24 PF' },
      ],
    },
    {
      short: 'IMD',
      ...
      ministry: 'Ministry of Earth Sciences, Government of India',
      logo: '/src/assets/emblem-india.svg',
      accent: '#1a5fa8',
      website: 'https://mausam.imd.gov.in',
      description: [
        'The India Meteorological Department (IMD) is the National Meteorological Service of India and the principal government agency in all matters relating to meteorology, seismology and allied subjects. Founded in 1875, IMD is one of the oldest meteorological services in the world.',
        'IMD provides weather forecasts, warnings, and advisories across the country through its extensive network of surface, upper air, radar, and satellite observation systems. It issues cyclone track and intensity forecasts, monsoon outlook, agrometeorological advisories, and aviation weather services.',
        "With headquarters in New Delhi and regional offices across India, IMD operates over 550 surface observatories, 39 radiosonde stations, 30+ Doppler Weather Radars, and receives data from ISRO's INSAT/EOS satellite constellation. IMD is designated as a Regional Specialised Meteorological Centre (RSMC) for tropical cyclones in the North Indian Ocean by the World Meteorological Organization (WMO).",
        'IMD plays a pivotal role in national disaster preparedness through timely warnings for cyclones, heavy rainfall, floods, heat waves, thunderstorms, and other severe weather events, directly saving thousands of lives annually.',
      ],
      highlights: [
        { label: 'Established', value: '1875' },
        { label: 'HQ', value: 'New Delhi' },
        { label: 'WMO Role', value: 'RSMC — N. Indian Ocean' },
        { label: 'Network', value: '550+ Observatories' },
      ],
    },
  ]
  ...
  return (
    <div className="min-h-screen bg-[#f0f5fc] pt-28 pb-16">
      ...
      <div className="bg-gradient-to-r from-[#021b3e] to-[#0b2d5e] py-16 border-b border-sky-700/30">
        <div className="max-w-5xl mx-auto px-4 sm:px-6">
          <p className="font-sans text-xs font-bold uppercase tracking-[.18em] text-amber-400 mb-2">Supporting Institutions</p>
          <h1 className="font-display text-4xl font-bold text-white mb-3">Institutional Patrons</h1>
          <div className="w-10 h-0.5 bg-amber-400 mb-4" />
          <p className="text-sky-300 font-sans text-sm max-w-2xl">
            TROPMET 2026 is proudly supported by leading national institutions dedicated to advancing meteorological science in India.
          </p>
        </div>
      </div>
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 mt-12 space-y-16">
        {patrons.map((p, idx) => (
          <div key={p.short} className="bg-white rounded-3xl shadow-lg overflow-hidden border border-[#c8d9f0]">
            {/* Coloured top bar */}
            <div className="h-2" style={{ background: `linear-gradient(to right, ${p.accent}, #1a5fa8)` }} />

            <div className="p-8 md:p-10">
              {/* Header row */}
              <div className="flex flex-col sm:flex-row items-start sm:items-center gap-6 mb-8 pb-8 border-b border-[#e6effa]">
                <div className="w-24 h-24 shrink-0 bg-[#f0f5fc] rounded-2xl flex items-center justify-center p-3 shadow-inner border border-[#c8d9f0]">
                  <img src={p.logo} alt={p.short} className="w-full h-full object-contain" />
                </div>
                <div className="flex-1">
                  <span className="inline-block text-[11px] font-bold uppercase tracking-widest bg-amber-100 text-amber-700 border border-amber-200 px-3 py-1 rounded-full mb-2">
                    Institutional Patron
                  </span>
                  <h2 className="font-display text-2xl font-bold text-[#0b2d5e] mb-1">{p.name}</h2>
                  <p className="text-slate-500 text-sm font-sans">{p.ministry}</p>
                </div>
                <a href={p.website} target="_blank" rel="noopener noreferrer"
                  className="shrink-0 inline-flex items-center gap-2 bg-[#0b2d5e] hover:bg-[#0e3d7a] text-white text-xs font-bold px-4 py-2.5 rounded-lg transition-colors shadow-sm">
                  Visit Website <ChevronRight size={13} />
                </a>
              </div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-8">
                {p.highlights.map((h) => (
                  <div key={h.label} className="bg-[#f0f5fc] border border-[#c8d9f0] rounded-xl p-4 text-center">
                    <div className="font-display font-bold text-[#0b2d5e] text-sm mb-0.5">{h.value}</div>
                    <div className="text-slate-400 text-[11px] font-sans uppercase tracking-wide">{h.label}</div>
                  </div>
                ))}
              </div>
              ...
              <div className="space-y-4">
                {p.description.map((para, i) => (
                  <p key={i} className="text-slate-600 font-sans text-sm leading-relaxed">{para}</p>
                ))}
              </div>
            </div>
          </div>
        ...
        <div className="bg-amber-50 border border-amber-200 rounded-2xl p-6 text-center">
          <p className="text-amber-800 font-sans text-sm leading-relaxed">
            For partnership and patronage enquiries, contact{' '}
            <a href="mailto:imsnoidachapter@gmail.com" className="font-bold underline">imsnoidachapter@gmail.com</a>
          </p>
        </div>
      </div>
