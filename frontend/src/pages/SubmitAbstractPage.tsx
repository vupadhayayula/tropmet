...
import { Upload, CheckCircle, FileText, User, BookOpen, ChevronRight, ChevronLeft, Check, Clock, Lock } from 'lucide-react'
...
// Login guard — in a real app this would check auth context/token
function LoginGuard({ children }: { children: React.ReactNode }) {
  // For demo purposes: show a gate page. In production connect to auth context.
  const [authorized] = useState(false) // Change to true when auth context is wired
  if (!authorized) {
    return (
      <div className="min-h-screen bg-[#f0f5fc] pt-28 pb-16 px-4 flex items-center justify-center">
        <div className="max-w-md w-full bg-white rounded-2xl border border-[#e6effa] shadow-sm p-10 text-center">
          <div className="w-14 h-14 bg-[#0b2d5e] rounded-2xl flex items-center justify-center mx-auto mb-4">
            <Lock size={24} className="text-amber-400" />
          </div>
          <h2 className="font-display font-bold text-2xl text-[#0b2d5e] mb-2">Login Required</h2>
          <p className="text-slate-500 text-sm mb-6">
            Only registered and logged-in authors can submit abstracts.<br />
            Please register first, then login from your author dashboard.
          </p>
          <div className="space-y-3">
            <Link to="/author-dashboard"
              className="w-full bg-[#0b2d5e] hover:bg-[#0e3d7a] text-white py-2.5 rounded-xl font-bold text-sm flex items-center justify-center gap-2 transition-colors">
              Login to Dashboard
            </Link>
            <Link to="/author-registration"
              className="w-full border border-[#c8d9f0] text-[#0b2d5e] hover:bg-[#eef4fc] py-2.5 rounded-xl font-semibold text-sm flex items-center justify-center gap-2 transition-colors">
              Register as Author
            </Link>
          </div>
          <div className="mt-5 bg-amber-50 border border-amber-200 rounded-xl px-4 py-3 text-xs text-amber-700 text-left">
            ...
            <ol className="mt-1 space-y-0.5 list-decimal list-inside">
              <li>Register your author account</li>
              <li>Login using your credentials</li>
              <li>Click "Submit Abstract" in your dashboard</li>
            </ol>
          </div>
        </div>
      </div>
...
export default function SubmitAbstractPage() {
  return <LoginGuard><SubmitAbstractPageInner /></LoginGuard>
}

function SubmitAbstractPageInner() {
